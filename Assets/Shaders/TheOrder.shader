Shader "Cloth/TheOrder"
{
    ...
    SubShader
    {
		...
		#pragma surface surf TheOrder fullforwardshadows
		...
		half NDF(half NdotH)
		{
			...
			half sin2h = 1. - cos2h;
			half amplitude = 4.;
			half cot2h = cos2h / sin2h;
			half sin4h = sin2h * sin2h;
			half ef = exp(-cot2h/ r2);
			half cNorm = 1. / (pi * (1. + (amplitude * r2)));
			return cNorm * (1.+((amplitude * ef) / sin4h));
		}

		
		inline half4 LightingTheOrder(SurfaceOutput s, half3 lightDir, half3 viewDir, half atten) {
			...
			//NDF//
			//Diffuse//
			half diff = s.Albedo*(1. - fresnel) / pi;
			//half diff = NdotL; // Lambert
			//Diffuse//
			//Specular//
			...
			c.rgb = (_LightColor0.rgb * diff + _LightColor0.rgb * specular) * atten;
			//c.rgb = specular * _LightColor0;
			c.a = s.Alpha;
			return c;
		}
